...
{ 
	...
	{
		$icons[] = array('value' => 'fab '.$match[1], 'label' => $match[1]);
	}
...
function vervoer_the_pagination2($args = array(), $echo = 1)
{
...
add_filter('vervoer_redux_custom_fonts_load', 'vervoer_redux_custom_fonts_load');
function vervoer_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	...
	if ( $filename = vervoer_set( $pathinfo, 'filename' ) ) {
		...
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/vervoer/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [vervoer_social_share_output description]
 *
 ...
 */
function vervoer_social_share_output( $icon, $color = false ) {
	...
	if ( $icon == 'digg' ) {
		...
		<li>
			<a onClick="window.open('http://www.digg.com/submit?url=<?php echo esc_url( $permalink ); ?>', 'Digg', 'width=715,height=330,left=' + (screen.availWidth / 2 - 357) + ',top=' + (screen.availHeight / 2 - 165) + '');
				...
				<i class="fab fa-digg" <?php echo wp_kses( $digg, $allowed_html ); ?>></i>
			</a>
		</li>
	<?php } ?>
	<?php
	if ( $icon == 'reddit' ) {
		...
		<li>
			<a onClick="window.open('http://reddit.com/submit?url=<?php echo esc_url( $permalink ); ?>&amp;title=<?php echo str_replace( " ", "%20", $titleget ); ?>', 'Reddit', 'width=617,height=514,left=' + (screen.availWidth / 2 - 308) + ',top=' + (screen.availHeight / 2 - 257) + '');
				...
				<i class="fab fa-reddit" <?php echo wp_kses( $reddit, $allowed_html ); ?>></i>
			</a>
		</li>
	<?php } ?>
	<?php
	if ( $icon == 'linkedin' ) {
		...
		<li>
			<a onClick="window.open('http://www.linkedin.com/shareArticle?mini=true&amp;url=<?php echo esc_url( $permalink ); ?>', 'Linkedin', 'width=863,height=500,left=' + (screen.availWidth / 2 - 431) + ',top=' + (screen.availHeight / 2 - 250) + '');
				...
				<i class="fab fa-linkedin" <?php echo wp_kses( $linkeding, $allowed_html ); ?>></i>
			</a>
		</li>
	<?php } ?>
	...
	if ( $icon == 'stumbleupon' ) {
		...
		<li>
			<a onClick="window.open('http://www.stumbleupon.com/submit?url=<?php echo esc_url( $permalink ); ?>&amp;title=<?php echo str_replace( " ", "%20", $titleget ); ?>', 'Stumbleupon', 'width=600,height=300,left=' + (screen.availWidth / 2 - 300) + ',top=' + (screen.availHeight / 2 - 150) + '');
				...
				<i class="fab fa-stumbleupon" <?php echo wp_kses( $stumbleupon, $allowed_html ); ?>></i>
			</a>
		</li>
	<?php } ?>
	<?php
	if ( $icon == 'tumblr' ) {
		...
		<li>
			<a onClick="window.open('http://www.tumblr.com/share/link?url=<?php echo esc_attr( $str ); ?>&amp;name=<?php echo str_replace( " ", "%20", $titleget ); ?>', 'Tumblr', 'width=600,height=300,left=' + (screen.availWidth / 2 - 300) + ',top=' + (screen.availHeight / 2 - 150) + '');
				...
				<i class="fab fa-tumblr" <?php echo wp_kses( $tumblr, $allowed_html ); ?>></i>
			</a>
		</li>
	<?php } ?>
	<?php
...
function vervoer_get_social_icons()
{
	...
	if( vervoer_set( $options, 'social_media' ) && is_array( vervoer_set( $options, 'social_media' ) ) )
	{
		$total = count( vervoer_set( $options, 'social_media' ) ) - 2;
		foreach( vervoer_set( $options, 'social_media' ) as $social_icon ){
			if( isset( $social_icon['tocopy' ] ) ) continue;
			$title = vervoer_set( $social_icon, 'title');
			$class = vervoer_set( $social_icon, 'class');
			$link = vervoer_set( $social_icon, 'social_link');
			$icon = vervoer_set( $social_icon, 'social_icon');
...
add_action('vervoer_social_share_output', 'vervoer_social_share_output');
...
add_action('vervoer_social_share_output_two', 'vervoer_social_share_output_two');
