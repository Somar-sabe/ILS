...
class Options {
	 ...
	 * @var string
	 */
	private $opt_name = 'vervoer' . '_options';
	...
	private $page_slug = 'vervoer' . '_options';
	...
	function init() {
		...
		$this->menu_title = esc_html__( 'VERVOER Options', 'vervoer' );
		$this->page_title = esc_html__( 'VERVOER Options', 'vervoer' );
	...
	function args() {
		...
		$args = array(
			...
			'global_variable'      => 'vervoer_options',
	...
	function helpTabs() {
		...
		$tabs = array(
			array(
				'id'      => 'redux-help-tab-1',
				'title'   => esc_html__( 'Theme Information 1', 'vervoer' ),
				'content' => esc_html__( '<p>This is the tab content, HTML is allowed.</p>', 'vervoer' ),
			),
			array(
				'id'      => 'redux-help-tab-2',
				'title'   => esc_html__( 'Theme Information 2', 'vervoer' ),
				'content' => esc_html__( '<p>This is the tab content, HTML is allowed.</p>', 'vervoer' ),
			),
		);
		...
		$content = '<p>' . esc_html__( 'This is the sidebar content, HTML is allowed.', 'vervoer' ) . '</p>';
	...
	function sections() {
		...
		$sections_path = apply_filters( 'vervoer_redux_sections', $sections_path );
		...
		foreach ( $sections_path as $key => $file ) {
			...
			if ( file_exists( $file ) ) {
				...
				$options = apply_filters( "vervoer_redux_sections_{$key}", $options );
