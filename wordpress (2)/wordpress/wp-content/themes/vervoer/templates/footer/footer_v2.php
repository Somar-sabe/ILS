...
$footer_shape_3    = vervoer_set( $footer_shape_3, 'url', VERVOER_URI . 'assets/images/shape/footer-shape-1.png' );

$footer_shape_4    = $options->get( 'footer_shape_four' );
$footer_shape_4    = vervoer_set( $footer_shape_4, 'url', VERVOER_URI . 'assets/images/shape/footer-shape-2.png' );
                    ...
                    <div class="left pull-left">
                        <div class="subscribe-social-links">
                            ...
                            <?php
								...
								$header_social_icons = json_decode( urldecode( vervoer_set( $h_icon, 'data' ) ) );
								if ( vervoer_set( $header_social_icons, 'enable' ) == '' ) {
									continue;
								}
								$icon_class = explode( '-', vervoer_set( $header_social_icons, 'icon' ) );
								?>
								<li><a href="<?php echo esc_url(vervoer_set( $header_social_icons, 'url' )); ?>" style="background-color:<?php echo esc_attr(vervoer_set( $header_social_icons, 'background' )); ?>; color: <?php echo esc_attr(vervoer_set( $header_social_icons, 'color' )); ?>"><i class="fa <?php echo esc_attr( vervoer_set( $header_social_icons, 'icon' ) ); ?>"></i></a></li>
							<?php endforeach; ?>
                            </ul>
                        </div>    
                    </div>
