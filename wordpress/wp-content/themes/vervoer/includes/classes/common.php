...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => vervoer_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => vervoer_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => vervoer_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => vervoer_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => vervoer_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => vervoer_meta( 'banner_elementor_template', $page_id ),
				'banner'            => vervoer_set( vervoer_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => vervoer_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => vervoer_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : vervoer_the_title( $template ),
				'banner'        => vervoer_set( $bg, 'url' ),
