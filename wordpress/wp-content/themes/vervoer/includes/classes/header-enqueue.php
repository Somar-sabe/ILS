...
class Header_Enqueue {
	...
	public static function scripts() {
		$options = get_theme_mod( 'vervoer' . '_options-mods' );
		$ssl     = is_ssl() ? 'https' : 'http';

		$scripts = array(
			
		);
		$scripts = apply_filters( 'VERVOER/includes/classes/header_enqueue/scripts', $scripts );
		...
		$header_data = array(
			...
			'nonce'   => wp_create_nonce( VERVOER_NONCE ),
		);

		wp_localize_script( 'jquery', 'vervoer_data', $header_data );

		if ( vervoer_set( $options, 'footer_js' ) ) {

			wp_add_inline_script( 'jquery', vervoer_set( $options, 'footer_js' ) );
		}
	...
	public static function styles() {
		 ...
		 wp_enqueue_style( 'main-color2', get_template_directory_uri() . '/assets/css/color.php?main_color2='.$maincolor2 );
		 
		 */
		
		$styles = array(
		);		
		$styles = apply_filters( 'VERVOER/includes/classes/header_enqueue/styles', $styles );
		foreach ( $styles as $name => $style ) {
			if ( strstr( $style, 'http' ) || strstr( $style, 'https' ) || strstr( $style, 'fonts.googleapis' ) ) {
				wp_enqueue_style( "vervoer-{$name}", $style );
			} else {
				wp_enqueue_style( "vervoer-{$name}", get_template_directory_uri() . '/' . $style );
			}
		}
		$options      = vervoer_WSH()->option();
		$custom_style = '';
		...
		if ( $custom_font = $options->get('theme_custom_font') ) {
            $header_styles .= vervoer_custom_fonts_load( $custom_font );
        }

		wp_add_inline_style( 'vervoer-main-style', $header_styles );
	}
	...
	public static function resource_hints( $urls, $relation_type ) {
		if ( wp_style_is( 'vervoer-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
			$urls[] = array(
	...
	public static function header_styles() {
		...
		$settings = get_theme_mod( 'vervoer' . '_options-mods' );
		if ( $custom_font = vervoer_set( $settings, 'theme_custom_font' ) ) {
			$styles .= apply_filters('vervoer_redux_custom_fonts_load', $custom_font );
		}
		return $styles;
	}
}